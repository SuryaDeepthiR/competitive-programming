...
def find_second_largest(root):
    ...
    if  (not root or not (root.right or root.left)) :
        raise Exception('There is no second elemnet to find second largest')
    return find_second_largest_t(root)

def find_second_largest_t(root, head = None):
    if(not root.right):
        if (root.left):
            return find_first_largest(root.left)
        return head
    return find_second_largest_t(root.right, root.value)

def find_first_largest(root):
    if(not root.right):
        return root.value
    return find_first_largest(root.right)
